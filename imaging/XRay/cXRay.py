...
class XRay(ImageSample):
    ...
    def get_section(self, section_lower, section_upper=None, section_length=5):
        assert self.depth_section is not None, 'specify the depth section if not loaded'
        if section_upper is None:
            section_upper = section_lower + section_length
        roi = self.sget_sample_area()[0]
        section_core = self.depth_section[1] - self.depth_section[0]
        # indices corresponding to depth
        w = self.xywh_ROI[2]
        mask = slice(
            int(w * (section_lower - self.depth_section[0]) / section_core + .5),
            int(w * (section_upper - self.depth_section[0]) / section_core + .5)
        )
        ...
        if self.plts:
            ...
            plt.gca().axis('off')
            plt.title(f'Section between {section_lower} and {section_upper} cm')
            plt.show()
    ...
    def remove_bars(self, n_sections = 10):
        ...
        for i_section in range(n_sections):
            section_lower = self.depth_section[0] + i_section * section_length
            image_section = ensure_image_is_gray(self.get_section(
                section_lower, section_length=section_length
            ))
