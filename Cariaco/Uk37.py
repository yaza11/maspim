...
folders = [
    r'C:\Users\Yannick Zander\Promotion\Cariaco 2024/490-495cm/2018_08_27 Cariaco 490-495 alkenones.i',
    r'C:\Users\Yannick Zander\Promotion\Cariaco 2024/495-500cm/2018_08_28 Cariaco 495-500 alkenones.i',
    r'C:\Users\Yannick Zander\Promotion\Cariaco 2024/500-505cm/2018_08_29 Cariaco 500-505 alkenones.i',
    r'C:\Users\Yannick Zander\Promotion\Cariaco 2024/505-510cm/2018_08_30 Cariaco 505-510 alkenones.i'
]
...
specs.full_targeted(readers=readers, targets=[mC37_3, mC37_2], integrate_peaks=False, SNR_threshold=4)
...
uk_combined = UK37(ts_combined)
uk_combined.correct(correction_factor=1.194)
uk_combined.add_SST(method='BAYSPLINE')
...
uk_age = uk_combined.feature_table.age
uk_uk = uk_combined.feature_table.ratio
uk_SST = uk_combined.feature_table.SST


mask = (df.age >= uk_age.min()) & (df.age <= uk_age.max())

# %% UK val
# add standard errors
errs = uk_combined.get_feature_table_standard_errors()

errs_uk = uk_combined.get_std_err()

plt.figure()
plt.fill_between(uk_age, uk_uk + errs_uk, uk_uk - errs_uk, alpha=.5, label='SE')
plt.plot(
    uk_age, 
    uk_uk, 
    label='automatic'
...
for idx, age_bin_center in enumerate(df.age[mask]):
    mask_bin = (uk_combined.feature_table.age >= age_bin_center - bin_width / 2) &\
        (uk_combined.feature_table.age < age_bin_center + bin_width / 2)
    binned_SSTs[idx] = uk_combined.feature_table.SST[mask_bin].mean()
    

plt.figure()
plt.fill_between(uk_age, uk_combined.feature_table.SST_lower, uk_combined.feature_table.SST_upper, alpha=.5, label='SE')
plt.plot(
    uk_age, 
    uk_SST, 
    label='automatic'
...
plt.plot(df.age[mask], df.SST[mask], label='paper')
plt.xlabel('Age in yrs b2k')
...
df_uks = pd.DataFrame({'automatic': binned_SSTs, 'paper': df.SST[mask]})
corr = df_uks.corr()
